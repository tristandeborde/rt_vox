    ...
    const int &depth)
{
    ...
    // find intersection of this ray with the cube in the scene
    ...
    Vec3f nhit = phit - cube->center; // normal at the intersection point
    ...
    if ((cube->transparency > 0 || cube->reflection > 0) && depth < MAX_RAY_DEPTH) {
        ...
        for (unsigned i = 0; i < cubes.size(); ++i) {
            if (cubes[i].emissionColor.x > 0) {
                ...
                Vec3f lightDirection = cubes[i].center - phit;
